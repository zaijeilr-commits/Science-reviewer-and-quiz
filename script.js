...
// Simple Icon Components for Browser
        ...
        {view === 'summary' ? (
          ...
          <div className="space-y-6">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-200">
              <h2 className="text-xl font-black mb-6">Metals vs. Non-metals (Page 19)</h2>
              <table className="w-full text-xs text-left">
                ...
                <tbody>
                  ...
                  <tr><td className="p-3 border-b font-bold">Conduction</td><td className="p-3 border-b">Good</td><td className="p-3 border-b">Insulators</td></tr>
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden">
            ...
            <div className="p-10 md:p-16">
              <h3 className="text-2xl font-black text-slate-800 mb-10">{questions[currentQuestion].q}</h3>
                ...
                {questions[currentQuestion].options.map((opt, i) => {
                  ...
                  return (
                    <button key={i} disabled={showExplanation} onClick={() => handleOptionClick(i)} className={btnClass}>
                      <span>{opt}</span>
                      {showExplanation && isCorrect && <Check />}
                      {showExplanation && isSelected && !isCorrect && <X />}
                    </button>
                  );
                })}
              </div>
              {showExplanation && (
                <div className="bg-slate-900 text-slate-100 p-6 rounded-3xl mb-10 flex gap-4">
                  <Lightbulb />
                  <p className="text-sm"><b>Review:</b> {questions[currentQuestion].ex}</p>
                </div>
              )}
              <div className="flex justify-between items-center pt-8 border-t">
                <button onClick={prevQuestion} className={`font-black text-slate-400 ${currentQuestion === 0 ? 'invisible' : ''}`}>Back</button>
                <button onClick={nextQuestion} disabled={selectedOption === null} className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black">
          ...
          <div className="bg-white rounded-[3.5rem] shadow-2xl p-16 text-center">
            <h2 className={`text-6xl font-black mb-4 ${isPassed ? 'text-green-600' : 'text-red-600'}`}>{isPassed ? 'PASSED' : 'FAILED'}</h2>
            <p className="text-slate-400 mb-12 text-xl font-bold">Score: {finalScore}/30</p>
            <button onClick={() => window.location.reload()} className="bg-slate-900 text-white px-12 py-6 rounded-[2rem] font-black">Restart</button>
          </div>
        )}
      </div>
    </div>
